...
| `services/usePaginatedPosts.ts` | `services/useBooks.ts` (plus `useGenres`) |
...
| `components/PostList.tsx` (`PostCard`) | `components/BookCard.tsx` |
...
`BookViewSet` follows the same shape — `IsAuthenticatedOrReadOnly`, `lookup_field = "slug"`, drafts filtered on every route — with three differences worth knowing:

- **`?genre=` is case-insensitive and an unknown value is *not* an error**, which is the opposite of `?category=`. Genres are free text, so there is no list to typo relative to and an unused label legitimately matches nothing. Case-insensitivity takes two steps because an `ArrayField` has no `icontains` lookup: `_stored_genres()` finds the spellings that match, then the filter asks for books carrying any of them (`genres__overlap`). `__contains` alone is exact, so a filter naming "Sci-Fi" would miss a book filed under "sci-fi".
- **`GET /api/books/genres/` folds spellings together, which the stored values are not.** `clean_labels` dedupes *one book's* genres case-insensitively but keeps what was typed, so "Sci-Fi" and "sci-fi" are two distinct strings across two books. Offering both as filter options would be offering the same filter twice with its count split between them, so the commonest spelling wins and the counts are summed (ties broken alphabetically, so the answer does not depend on row order). It is scoped by `get_queryset()`, so an anonymous caller is never offered a genre only a draft carries.
...
**`list_filter` on an `ArrayField` has to be hand-written** — `CategoryFilter` for a post's categories and `GenreFilter` for a book's genres, both in `admin.py`. Django builds an exact-match filter from a plain `list_filter` entry, so on an `ArrayField` the sidebar would offer whole combinations — "books, projects" — as single values and match nothing else. Each `SimpleListFilter` performs the same containment test its API parameter does. `GenreFilter` builds its options from the rows themselves, since a genre is free text and there is no enum to read them off.

### Tests

`myapp/tests.py` is a real suite (159 tests, `APITestCase`) covering slug generation, publish stamping, the draft visibility rules, filter validation, basic-auth writes, each CRUD verb for both anonymous and authenticated callers, the upload endpoint, the view counter, the daily reading history (`ViewDayTests`), the date-range filter, and the book catalogue (`BookModelTests` / `BookAPITests` — slug derivation and its author fallback, genre and ISBN normalisation, both check-digit schemes, the release-year bounds, the case-insensitive genre filter, the genres action's spelling fold, and each CRUD verb for both anonymous and authenticated callers). Run it with `python manage.py test`.
...
`/` (the Posts feed), `/projects` and each one's `/…/:slug` detail route render live data from `GET /api/posts/`. **`/books` is the exception: it is the catalogue, backed by `GET /api/books/`, not the feed filtered to `category=books`** — so `/books/:slug` renders `BookDetail` over a `Book`, while a post filed under the books category still reaches its page at `/posts/:slug` through `CROSS_CATEGORY_BASE_PATH`. The CV and About pages are the only hardcoded copy left.
