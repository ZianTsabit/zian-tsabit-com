...
- **Unmounting flushes a pending save rather than cancelling it**, the same reasoning as `useRecordView`: leaving the page is exactly when the last few seconds of typing would otherwise be lost. Only the state update is skipped once the component is gone.
